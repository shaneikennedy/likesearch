...
def search_likes(likes: Sequence[Like], q: str) -> Sequence[Like]:
    return [l for l in likes if l.text.lower().find(q) >= 0]
...
def get_oldest_like(likes: Sequence[Like]) -> Like:
    ...
    return reduce(lambda l, oldest: l if l.id < oldest.id else oldest, likes, likes[0])
...
def get_cli_args():
    ...
    cli_parser.add_argument(
        "--first",
        default=True,
        help="Stop after the first hit(s)",
        choices=[True, False],
    )
